...
class Store(private var elems: ArrayBuffer[Value] = ArrayBuffer[Value]()) extends Value {
  ...
  def rem(pos: Integer) { elems.remove(pos.value, 1) }
  ...
  def map(trans: Closure): Store = { new Store(elems.map(e => trans.apply(List(e)))) }
  
  // returns store containing the elements of this that passed test
  def filter(test: Closure): Store = { new Store(elems.filter(e => (test.apply(List(e)).isInstanceOf[Boole] && test.apply(List(e)).asInstanceOf[Boole].value))) }
}